...
/** Virtualized cover grid, generic over the item type. Shared by the series and
 *  the flat-issues browse — both render `aspect-[2/3]` cover cards, so the column
 *  geometry and scroll-restore logic are identical; only the card differs. */
export function CardGrid<T>({ items, getKey, renderItem, density, hasNext, fetchNext, initialIndex, onTopIndex }: {
  items: T[]; getKey: (item: T) => string; renderItem: (item: T) => React.ReactNode
  density: Density; hasNext: boolean; fetchNext: () => void
